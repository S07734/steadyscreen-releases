...
def audio_test():
    # The tone runs about six seconds; the caller must not use a short timeout.
...
async function tick(){
  ...
  lede.textContent = d.phase === 'ready'
      ? 'This display is ready. It has no page to show yet.'
...
class DisplayClient:
    def __init__(self, cfg):
        ...
        # Hide the cursor only when nothing can move it. Hiding it on a
        # machine with a mouse attached makes the mouse look broken: it works,
        # you just cannot see where you are pointing. That cost real time in
        # the field on the network screen.
        if cfg["hide_cursor"] and not input_devices()["pointer"]:
...
<div class=card>
 ...
 <h2 style="margin-top:1.2rem">Saved networks</h2>
 <div id=savedlist class=net></div>
 <h2 style="margin-top:1.2rem">In range</h2>
 <div id=wifilist></div>
...
<div class=card>
 ...
 <div class=grid2 style="margin-top:.9rem">
  ...
  <div><label for=geo>Location services</label>
...
<div class=card>
 ...
 <div class=row>
  ...
  <button class=danger onclick="confirmPost('/api/shutdown','Shut down the device?')">Shut down</button>
 </div>
</div>
</div>
<script>
...
const TABS = [
  ...
  ['display',  'Display', ['Display', 'Boot splash']],
  ['system',   'System',  ['Printing', 'Audio', 'Clock',
                          'Remote access (SSH)',
...
function buildTabs(){
  ...
  for(const c of byTitle.values()) first.appendChild(c);
...
async function refresh(){
  ...
  host.textContent =
    `${s.client_name || s.kiosk_name || s.hostname} · ${s.ip} · SteadyScreen ${s.version}`;
  ...
  const phase = {'ready':'<span class=ok>page loaded</span>',
...
function audioTest(){
  ...
  audioPost({action:'test'}, 'Playing a tone (about six seconds)...');
}
...
async function refreshPhase(){
  let s; try { s = await (await fetch('/api/status')).json(); } catch(e){ return; }
  const nice = {'ready':'<span class=ok>page loaded</span>',
...
<div class=card id=admincard style="display:none">
  <h2>Change this device</h2>
  <div style="font-size:1.05rem">
    Open <b><span id=adminurl></span></b> from a phone or laptop on this
    network to change the page address or anything else.
  </div>
  <div style="color:var(--dim);font-size:.9rem;margin-top:.5rem">
    On this screen, <b>Ctrl+Alt+A</b> opens the same page.
    <span id=stuckwhy></span>
  </div>
</div>
...
function applyInputMode(inp){
  ...
  // a real keyboard means the on-screen one is just clutter
...
async function refresh(){
  ...
  // The admin address, as soon as there is one. Being stuck on this screen
  // with a page that will never answer is survivable; being stuck with no
  // idea how to reach the settings is what forces a reflash.
  const ac = document.getElementById('admincard');
  if(ac){
    if(d.admin_ip){
      document.getElementById('adminurl').textContent =
        'http://' + d.admin_ip + ':' + (d.admin_port || 8080);
      document.getElementById('stuckwhy').innerHTML =
        d.configured === false
          ? '<br>Nothing else needs setting up at the machine &mdash; the page '
            + 'address, name, timezone and everything else are all there.'
          : (d.gave_up
              ? '<br>The configured page did not answer, so this screen stays '
                + 'up. Its address is <b>' + esc(d.url || '') + '</b>.'
              : '');
      ac.style.display = '';
    } else {
