...
def clock_update(box):
    ...
    # sort of thing that gets reported as a fault.
    want = (time.strftime("%-I:%M %p", now), time.strftime("%A %-d %B", now))
    changed = False
...
<div class=card>
 ...
 <div class=hint id=saverblankhint>Both times count from the last touch, so
  ...
  never appears. A television shows &ldquo;no signal&rdquo; while off &mdash;
  try one display first.</div>
