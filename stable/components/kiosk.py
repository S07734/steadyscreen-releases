...
BUILTIN_PAGE_PATHS = ("/startup", "/welcome", "/cached")
...
class DisplayClient:
    ...
    def _idle_tick(self):
        ...
        if blank > 0 and allowed and idle > blank and not self.panel_off:
            self._panel_off()
        if reset > 0 and idle > reset:
            # Its own clock, so the screensaver still sees the true idle time.
            # This used to push last_input forward, which meant a display with
            # both settings on could never reach the longer of the two.
            if time.time() - getattr(self, "_last_reset", 0) > reset:
  ...
  <div><label for=rotation>Screen rotation</label>
   <select id=rotation style="width:100%;padding:.7rem .8rem;background:#111316;
    ...
    <!-- Windows' own words, in Windows' order, so it matches what people set
         ...
         panel and the touch matrix use; only the labels change. Windows
         "Portrait" is 90 deg clockwise (DMDO_90), which is xrandr "right";
         "Portrait (flipped)" is xrandr "left". -->
    <option value=normal>Landscape</option>
    <option value=right>Portrait</option>
    <option value=inverted>Landscape (flipped)</option>
    <option value=left>Portrait (flipped)</option></select></div>
...
const SETTING_FIELDS = [
  'url','zoom','refresh','idle','saver','saverstyle','saverwhen',
    ...
    def do_POST(self):                                        # noqa: N802
        ...
        if path == "/api/url":
            ...
            if status == 200 and reply.get("state") == "paired":
                ...
                k._hb_wake.set()
            self._json(200, {"server_status": status, "reply": reply})
