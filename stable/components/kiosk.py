...
def heartbeat_payload():
    ...
    and update channel ("yes add hostname and channel to the heartbeat"), and
    its private LAN address ("any reason we shouldn't include the local ip").
    Each optional field is left out when it cannot be read truthfully, which
    the server accepts. Nothing else -- not the fingerprint, not the page: the
    server refuses any other field with a 422."""
...
def token_verify(envelope, namespace, signers=None):
    ...
    want_type = {ACTIVATION_NS: "activation", LICENCE_NS: "licence"}.get(namespace)
    if not want_type:
...
const TABS = [
  ...
  ['system',   'System',  ['Status', 'Updates', 'Printing', 'Audio', 'Clock',
                          'Remote access (SSH)', 'Remote support', 'Rollback snapshots',
