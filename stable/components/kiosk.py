...
def os_version():
    """Which OS this slot is, as a plain integer.

    Anything non-numeric is a client from before the OS and the software had
    separate versions; it reports 0, which is older than every numbered OS and
    is exactly how the updater should treat it.
    """
    try:
...
FIRST_UPDATE_MARK = "/var/lib/kiosk/first-update-done"
...
def first_boot_update():
    ...
    while time.time() < deadline:
        ...
        try:
            os.makedirs(os.path.dirname(FIRST_UPDATE_MARK), exist_ok=True)
...
<h1 id=wordmark style="display:none">SteadyScreen</h1>
