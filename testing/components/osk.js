/* SteadyScreen on-screen keyboard.
 *
 * Injected into the configured page after it loads, for touch displays with no
 * physical keyboard. Touching a text field brings it up; it types into that
 * field the way a real keyboard would, so the page's own JavaScript sees
 * ordinary input and change events and cannot tell the difference.
 *
 * Deliberately self-contained and defensive: it runs inside somebody else's
 * page, and breaking their page is far worse than having no keyboard.
 */
(function () {
  if (window.__dsOsk) { return; }
  window.__dsOsk = 1;

  var COLS = 23;          /* 20 half-key columns, plus 3 for the right column */
  var MAIN = 20;

  /* Column starts are explicit rather than centred. A keyboard's stagger is
   * not symmetry: Z belongs half a key left of where centring puts it, so it
   * sits between A and S rather than under S. */
  var LAYERS = {
    abc: [{ k: '1234567890', c: 1 },
          { k: 'qwertyuiop', c: 1 },
          { k: 'asdfghjkl',  c: 2 },
          { k: 'zxcvbnm,.',  c: 3 }],
    sym: [{ k: '1234567890', c: 1 },
          { k: '!@#$%^&*()', c: 1 },
          { k: '-_=+[]{}:;', c: 1 },
          { k: ',./?\\|~',   c: 4 }]
  };

  var TEXTY = /^(text|search|tel|url|email|password|number|)$/i;
  function isField(el) {
    if (!el || !el.tagName) { return false; }
    var t = el.tagName.toLowerCase();
    if (t === 'textarea') { return true; }
    if (el.isContentEditable) { return true; }
    if (t !== 'input') { return false; }
    if (el.readOnly || el.disabled) { return false; }
    return TEXTY.test(el.getAttribute('type') || 'text');
  }

  /* Set by the client from config: a horizontal and vertical nudge in
   * pixels, for a screen with a dead patch under part of the keyboard. Found
   * on a real panel, where the same physical spot swallowed presses whichever
   * key happened to be sitting on it. */
  var OFF = window.__dsOskOffset || { x: 0, y: 0 };
  OFF = { x: parseInt(OFF.x, 10) || 0, y: parseInt(OFF.y, 10) || 0 };

  /* Key size, as a multiplier on the standard 56px row. Set from config in
   * fixed steps rather than continuously: every step has to be usable on real
   * glass, and a preset can be checked on a panel where a slider's thousand
   * values cannot. Clamped here too, because config is only as good as what
   * last wrote it. */
  function clampScale(v) {
    var n = parseFloat(v);
    if (!(n > 0)) { n = 1; }
    if (n < 0.8) { n = 0.8; }
    if (n > 2.2) { n = 2.2; }
    return n;
  }
  var SCALE = clampScale(window.__dsOskScale);
  var ENABLED = true;

  /* Auto-hide, and staying out of the way of a real keyboard.
   *
   * IDLE_MS: a keyboard left up over a page nobody is using is just a panel
   * covering half the display. Thirty seconds of nothing is long enough that
   * somebody reading the field before typing does not lose it.
   *
   * physical: a keyboard the display did not draw. Any keydown the BROWSER
   * marks trusted came from real hardware -- this keyboard never produces
   * one, because insert() uses execCommand or assigns .value, and the only
   * KeyboardEvents it dispatches are its own untrusted Enter.
   *
   * IT IS CLEARED BY THE NEXT TOUCH, deliberately, rather than latching for
   * the life of the page. A barcode scanner is a keyboard as far as the
   * browser is concerned, and these displays scan coupons -- latching would
   * mean one scan silently takes the keyboard away from every customer after
   * it, on exactly the machines that need it most. Clearing on touch costs
   * nothing to someone using a real keyboard, who is not touching the screen.
   */
  var IDLE_MS = 30000;
  var idleTimer = null;
  var physical = false;

  function idleStop() {
    if (idleTimer) { clearTimeout(idleTimer); idleTimer = null; }
  }

  /* Called by anything that counts as the keyboard still being in use. */
  function poke() {
    idleStop();
    if (!wrap || !wrap.classList.contains('on')) { return; }
    idleTimer = setTimeout(function () {
      idleTimer = null;
      hide();
    }, IDLE_MS);
  }
  /* Round to whole pixels. Half-pixel rows make the gaps between keys look
   * uneven on a cheap panel, which reads as a rendering fault. */
  function px(base) { return Math.round(base * SCALE); }

  /* The viewport ceiling has to grow with the setting, or it erases it.
   * A fixed 9vh cap meant that on a 1024x600 panel all four key sizes came
   * out byte-identical, and on 1366x768 three of the four did -- the admin
   * page offered four labels that were one setting. It looked correct only
   * on the 1920x1280 Pipo it was checked on, which is the tallest screen in
   * the fleet and the one place every step had room to differ.
   *
   * 9vh at standard, so small panels keep exactly the keys they have today,
   * rising to 11vh at huge. Five rows plus gaps and padding is then at worst
   * about two thirds of a short screen at the largest setting -- which is
   * what asking for the largest setting means. */
  function ceilVh(base) { return (base * (9 + (SCALE - 1) * 2.5) / 56).toFixed(2); }

  var DOWN = window.PointerEvent ? 'pointerdown'
           : ('ontouchstart' in window ? 'touchstart' : 'mousedown');

  var target = null, wrap = null, keysBox = null;
  var hostEl = null, shadow = null;
  var layer = 'abc';
  var caps = false;      /* caps lock */
  var oneShot = false;   /* shift for the next character only */
  var lastShift = 0;     /* for detecting a double tap on shift */

  function upper() { return caps || oneShot; }

  var styleEl = null;

  function cssText() {
    /* The '' + is load-bearing. `return` alone on its line would be given
       a semicolon by the parser and this function would return undefined --
       a keyboard with no styling at all, which is exactly what it did. */
    return '' +
      /* Every size is `min(<scaled px>, <vh>)`. The vh half is the ceiling:
         the ceiling rises with the key size, from 9vh a row at standard to
         11vh at huge, so the whole panel is at worst about two thirds of a
         short screen at the largest setting and under half on a tall one, so
         a large setting on a small panel shrinks to fit instead of burying the
         page. Doing it in CSS rather than JS means rotating the display or
         changing the resolution re-fits it with no code involved. */
      /* Selectors cannot cross a shadow boundary, but INHERITED properties
         still do -- font, colour, letter-spacing, text-transform, visibility,
         direction. `all:initial` on the host stops every one of them at the
         door, so a page with `body{text-transform:uppercase}` or a condensed
         font cannot reach into the keyboard. Everything the keyboard wants is
         declared below rather than inherited. */
      ':host{all:initial}' +
      '#ds-osk{position:fixed;left:0;right:0;bottom:0;z-index:2147483647;' +
      'background:#161a21;border-top:2px solid #2b313c;' +
      'padding:' + (px(10) + Math.max(0, OFF.y)) + 'px ' + px(10) + 'px ' + px(14) + 'px;' +
      'display:none;font:500 min(' + px(20) + 'px,' + ceilVh(20) + 'vh)/1 system-ui,-apple-system,Segoe UI,sans-serif;' +
      '-webkit-user-select:none;user-select:none;box-shadow:0 -8px 24px rgba(0,0,0,.5)}' +
      '#ds-osk.on{display:block}' +
      /* The key area can be nudged away from a dead patch on the digitiser.
         Only the KEYS move -- the panel stays full width, so a press that
         lands beside them is still inert and does not dismiss the keyboard. */
      '#ds-osk .keys{transform:translate(' + OFF.x + 'px,' + (-OFF.y) + 'px);' +
      /* minmax(0,1fr), not 1fr. `1fr` is `minmax(auto,1fr)`, and that
         `auto` floor is the item's min-content width -- so anything that
         gives a key a minimum makes the whole row wider than its container
         instead of the keys getting narrower. Belt and braces with the reset
         above: that fixes the cause we know about, this makes the row unable
         to overflow whatever a future page asks for. */
      'display:grid;grid-template-columns:repeat(' + COLS + ',minmax(0,1fr));' +
      'grid-auto-rows:minmax(min(' + px(56) + 'px,' + ceilVh(56) + 'vh),auto);' +
      'gap:min(' + px(7) + 'px,1vh);' +
      /* The width cap grows with the keys. Left at a fixed 1180px a larger
         setting made taller keys inside the same narrow block, which is not
         what "bigger keyboard" means to anyone looking at it. 96vw keeps it
         off the edges on a narrow screen. */
      'max-width:min(' + px(1180) + 'px,96vw);margin:0 auto}' +
      /* min-width and min-height are RESET, not just left unset. This
         keyboard is injected into a page nobody here wrote, and a page with
         its own `button { min-width: ... }` sizes every key from it: our own
         admin page sets 8.5rem, so every key was sized from it and the row
         grew past the cap. Measured on .63 (key size large, so the cap is
         1180*1.25 = 1475px): 1582px before, 1470px after. On the 1280-wide
         panel at FS Memphis, where the cap is 1180px, the same demand ran off
         the right-hand edge. Reported 2026-09-07.
         `width:auto` for the same reason -- a page that sets an explicit
         button width would otherwise size the keys too. */
      '#ds-osk button{border-radius:' + px(8) + 'px;border:1px solid #39434f;' +
      'border-bottom-width:3px;background:#242c37;color:#eef1f5;font:inherit;' +
      'padding:0;min-width:0;min-height:0;width:auto;box-sizing:border-box;' +
      'touch-action:manipulation}' +
      '#ds-osk button:active{background:#313c4a;border-bottom-width:1px;' +
      'transform:translateY(2px)}' +
      '#ds-osk .go{background:#2f6fd0;border-color:#4f8ae8}' +
      '#ds-osk .dim{background:#1b212a;color:#9aa6b4}' +
      '#ds-osk .tall{font-size:min(' + px(18) + 'px,' + ceilVh(18) + 'vh)}' +
      /* The bottom row's touch area runs to the bottom of the screen. The
         keys stay where they are; only the target grows. Pressing just below
         a key is a very easy thing to do on a wall-mounted panel, and it used
         to land on the page behind and dismiss the keyboard. */
      '#ds-osk .btm{position:relative}' +
      '#ds-osk .btm::after{content:"";position:absolute;left:0;right:0;' +
      'top:100%;height:' + px(60) + 'px}' +
      '';
  }

  function css() {
    styleEl = document.createElement('style');
    styleEl.textContent = cssText();
    /* Into the shadow root, not document.head. Inside the shadow the page's
       selectors cannot reach these elements at all. */
    shadow.appendChild(styleEl);
  }

  /* Re-style a keyboard that is already on the page. Settings used to be read
   * once, when the script was injected, so changing the key size did nothing
   * until the browser restarted -- a setting that appears to do nothing reads
   * as broken. The client calls this the moment one is saved. */
  function applyCfg(c) {
    if (!c) { return; }
    if (c.scale !== undefined) { SCALE = clampScale(c.scale); }
    if (c.offset) {
      OFF = { x: parseInt(c.offset.x, 10) || 0,
              y: parseInt(c.offset.y, 10) || 0 };
    }
    if (c.caps) {
      window.__dsOskCaps = c.caps;
      applyCapsMode();
      if (keysBox) { relabel(); }
    }
    if (c.enabled !== undefined) {
      ENABLED = !!c.enabled;
      if (!ENABLED) { hide(); }
    }
    /* Only the stylesheet carries the size, so rewriting it is the whole
     * update -- the keys keep their grid positions and their handlers. */
    if (styleEl) { styleEl.textContent = cssText(); }
  }
  window.__dsOskConfig = applyCfg;

  function key(label, cls, col, span, row, rowspan, fn) {
    var b = document.createElement('button');
    b.type = 'button';
    b.textContent = label;
    /* Never focusable. A button that can take focus pulls it off the text
     * field on a real tap, which fires focusout and used to hide the whole
     * keyboard -- most visibly when holding backspace. */
    b.tabIndex = -1;
    if (cls) { b.className = cls; }
    b.style.gridColumn = col + ' / span ' + span;
    if (row) { b.style.gridRow = row + ' / span ' + (rowspan || 1); }
    /* Exactly one event actually presses the key, chosen once for this
     * browser; the others are swallowed so they cannot move focus. Binding
     * three and deciding inside the handler meant a press could act once,
     * twice, or not at all depending on which events the engine delivered --
     * which is what made the symbols key feel unreliable under a finger. */
    b.addEventListener(DOWN, function (e) {
      e.preventDefault();
      e.stopPropagation();
      poke();
      fn(b);
      /* Compared against the function rather than the label: the glyph is a
       * presentation detail and has already changed once. */
      if (fn === backspace) { startRepeat(fn, b); }
    }, { passive: false });
    ['pointerdown', 'touchstart', 'mousedown', 'click'].forEach(function (ev) {
      if (ev === DOWN) { return; }
      b.addEventListener(ev, function (e) { e.preventDefault(); },
                         { passive: false });
    });
    return b;
  }

  function refocus() {
    if (target && document.activeElement !== target) {
      try { target.focus({ preventScroll: true }); } catch (err) {
        try { target.focus(); } catch (err2) {}
      }
    }
  }

  function insert(ch) {
    if (!target) { return; }
    refocus();
    try {
      if (target.isContentEditable) {
        document.execCommand('insertText', false, ch);
        return;
      }
      var s = target.selectionStart, e = target.selectionEnd;
      if (typeof s === 'number' && typeof e === 'number') {
        var v = target.value;
        target.value = v.slice(0, s) + ch + v.slice(e);
        target.selectionStart = target.selectionEnd = s + ch.length;
      } else {
        target.value += ch;
      }
      fire('input');
    } catch (err) { /* never break the page */ }
  }

  function backspace() {
    if (!target) { return; }
    refocus();
    try {
      if (target.isContentEditable) {
        document.execCommand('delete', false, null);
        return;
      }
      var s = target.selectionStart, e = target.selectionEnd, v = target.value;
      if (typeof s === 'number' && s !== e) {
        target.value = v.slice(0, s) + v.slice(e);
        target.selectionStart = target.selectionEnd = s;
      } else if (typeof s === 'number' && s > 0) {
        target.value = v.slice(0, s - 1) + v.slice(s);
        target.selectionStart = target.selectionEnd = s - 1;
      } else if (typeof s !== 'number') {
        target.value = v.slice(0, -1);
      }
      fire('input');
    } catch (err) { /* ignore */ }
  }

  /* Held keys repeat, like a physical keyboard -- and ONLY backspace asks
   * for it. A letter that repeated would turn a slightly long tap into
   * "aaaaa" on a touchscreen, which is why phone keyboards do not do it
   * either. Backspace is the opposite: without repeat, clearing a mistyped
   * URL is one poke per character.
   *
   * Chained setTimeout rather than setInterval: it lets the first gap be
   * long (so a normal tap never repeats) and the rest short, and there is
   * only ever one timer to cancel.
   */
  var REPEAT_DELAY = 420;      /* before the first repeat -- a tap must not */
  var REPEAT_EVERY = 55;       /* steady rate after that, about 18/second */
  var REPEAT_FAST = 28;        /* once it is clearly a deliberate hold */
  var REPEAT_RAMP = 18;        /* repeats before speeding up */
  var REPEAT_MAX = 8000;       /* hard stop; see stopRepeat below */
  var repeatTimer = null;

  function stopRepeat() {
    if (repeatTimer) { clearTimeout(repeatTimer); repeatTimer = null; }
  }

  function startRepeat(fn, b) {
    stopRepeat();
    var began = Date.now();
    var n = 0;
    var tick = function (wait) {
      repeatTimer = setTimeout(function () {
        /* A press whose release never arrives -- a finger dragged off the
         * panel, a pointercancel the engine swallowed, the keyboard hidden
         * mid-hold -- would otherwise sit here deleting the field forever.
         * The listeners below catch the normal cases; this catches the rest. */
        if (Date.now() - began > REPEAT_MAX) { stopRepeat(); return; }
        n += 1;
        fn(b);
        tick(n < REPEAT_RAMP ? REPEAT_EVERY : REPEAT_FAST);
      }, wait);
    };
    tick(REPEAT_DELAY);
  }

  /* Capture phase, on the window: the release very often does not land on
   * the button it started on, because a finger moves. Listening on the key
   * itself is what makes on-screen repeat stick down in every implementation
   * that gets this wrong. */
  ['pointerup', 'pointercancel', 'touchend', 'touchcancel', 'mouseup',
   'blur', 'visibilitychange'].forEach(function (ev) {
    window.addEventListener(ev, stopRepeat, true);
  });

  function fire(type) {
    try {
      target.dispatchEvent(new Event(type, { bubbles: true }));
    } catch (err) {
      try {
        var ev = document.createEvent('Event');
        ev.initEvent(type, true, true);
        target.dispatchEvent(ev);
      } catch (err2) {}
    }
  }

  function enter() {
    if (!target) { return; }
    refocus();
    fire('change');
    var opts = { bubbles: true, cancelable: true, key: 'Enter', code: 'Enter',
                 keyCode: 13, which: 13 };
    var prevented = false;
    try { prevented = !target.dispatchEvent(new KeyboardEvent('keydown', opts)); }
    catch (err) {}
    try { target.dispatchEvent(new KeyboardEvent('keypress', opts)); } catch (err) {}
    try { target.dispatchEvent(new KeyboardEvent('keyup', opts)); } catch (err) {}
    /* Only submit if the page did not handle Enter itself, or a scanner page
     * that listens for Enter would submit twice. */
    var f = target.form;
    if (!prevented && f) {
      try {
        if (typeof f.requestSubmit === 'function') { f.requestSubmit(); }
        else { f.submit(); }
      } catch (err) {}
    }
    hide();
  }

  /* Relabel the letter keys without rebuilding them. Rebuilding the DOM from
   * inside a key's own event handler destroys the element mid-dispatch, which
   * is a good way to make a key work only sometimes. */
  function relabel() {
    if (!keysBox) { return; }
    var bs = keysBox.querySelectorAll('button[data-ch]');
    for (var i = 0; i < bs.length; i++) {
      var c = bs[i].getAttribute('data-ch');
      bs[i].textContent = upper() ? c.toUpperCase() : c;
    }
    var sk = keysBox.querySelector('button[data-shift]');
    if (sk) {
      sk.textContent = caps ? '\u21ea' : '\u21e7';
      sk.className = 'btm ' + (caps ? 'go' : (oneShot ? '' : 'dim'));
    }
  }

  function shiftTapped() {
    var now = Date.now();
    var dbl = (now - lastShift) < 400;
    /* Consume the pair. Without this a third quick tap counts as another
     * double and toggles the lock straight back off, so two taps ended where
     * they started -- which is exactly what happened. */
    lastShift = dbl ? 0 : now;
    if (dbl) {
      caps = true;            /* a double tap LOCKS; it does not toggle */
      oneShot = false;
    } else if (caps) {
      caps = false;           /* a single tap releases the lock */
      oneShot = false;
    } else {
      oneShot = !oneShot;
    }
    relabel();
  }

  function render() {
    keysBox.innerHTML = '';
    var rows = LAYERS[layer];
    rows.forEach(function (row, i) {
      var chars = row.k.split('');
      var start = row.c;
      chars.forEach(function (c, j) {
        var b = key(upper() ? c.toUpperCase() : c, '', start + j * 2, 2,
                    i + 1, 1, function () {
                      insert(upper() ? c.toUpperCase() : c);
                      if (oneShot && !caps) { oneShot = false; relabel(); }
                    });
        b.setAttribute('data-ch', c);
        keysBox.appendChild(b);
      });
    });

    /* Backspace: right of 0 and p, two rows tall. Enter: under it, right of
     * the two letter rows, also two rows tall. */
    keysBox.appendChild(key('⌫', 'dim tall', MAIN + 1, 3, 1, 2, backspace));
    keysBox.appendChild(key('Enter', 'go tall', MAIN + 1, 3, 3, 2, enter));

    /* Bottom row, spanning all 23 columns: 3 + 17 + 3. The hyphen and full
     * stop have moved -- the stop next to M where a keyboard has it, the
     * hyphen into the symbol layer. */
    var sk = key(caps ? '⇪' : '⇧', 'btm ' + (caps ? 'go' : 'dim'), 1, 3, 5, 1,
                 shiftTapped);
    sk.setAttribute('data-shift', '1');
    keysBox.appendChild(sk);
    keysBox.appendChild(key('space', 'btm', 4, 17, 5, 1, function () { insert(' '); }));
    /* The rebuild is deferred to the next tick so this button still exists
     * while its own event finishes dispatching. */
    keysBox.appendChild(key(layer === 'abc' ? '?123' : 'ABC', 'btm dim', 21, 3, 5, 1,
                            function () {
                              layer = (layer === 'abc' ? 'sym' : 'abc');
                              setTimeout(render, 0);
                            }));
  }

  function build() {
    /* THE KEYBOARD LIVES IN A SHADOW ROOT.
     *
     * It is injected into pages nobody here wrote, and it inherits whatever
     * they declare. Our own admin page sets `button{min-width:8.5rem}`, which
     * sized every key from it and pushed the row off the right of a 1280-wide
     * panel -- reported from FS Memphis, 2026-09-07. That was one property,
     * and it was OUR page. A customer page could just
     * as easily set a font, a text-transform, or `button{display:none}`.
     *
     * A shadow root ends the whole class: page selectors cannot match
     * anything inside it. `closed` so the page cannot reach in through
     * .shadowRoot either. Inherited properties are stopped separately by
     * `:host{all:initial}` in the stylesheet, and the host's own visibility
     * is pinned inline below, because a page rule CAN still match the host
     * element itself. */
    hostEl = document.createElement('div');
    hostEl.id = 'ds-osk-host';
    ['display:block', 'visibility:visible', 'opacity:1',
     'pointer-events:auto', 'position:static', 'margin:0', 'padding:0',
     'border:0', 'width:auto', 'height:auto', 'max-width:none',
     'max-height:none', 'transform:none', 'filter:none', 'clip-path:none',
     'z-index:auto'].forEach(function (d) {
      var i = d.indexOf(':');
      hostEl.style.setProperty(d.slice(0, i), d.slice(i + 1), 'important');
    });
    shadow = hostEl.attachShadow({ mode: 'closed' });
    css();                       /* after the shadow: the stylesheet lives in it */

    wrap = document.createElement('div');
    wrap.id = 'ds-osk';
    /* Any press anywhere on the panel -- a key, the gap between keys, the
     * padding under the bottom row -- must not move focus off the field.
     * Without this, a press that missed a key landed on the page behind,
     * blurred the input, and the keyboard dismissed itself. Reported from a
     * real panel; a synthetic pointerdown on a button could never show it. */
    ['pointerdown', 'touchstart', 'mousedown'].forEach(function (ev) {
      wrap.addEventListener(ev, function (e) { e.preventDefault(); },
                            { passive: false });
    });
    keysBox = document.createElement('div');
    keysBox.className = 'keys';
    wrap.appendChild(keysBox);
    render();
    /* No close button. It dismisses the way a phone keyboard does: Enter
     * closes it, and so does touching anything on the page that is not a
     * text field. A button whose only job is to undo an accident is one more
     * thing to mis-tap on a panel this size.
     *
     * Note the panel still swallows presses that land beside the keys -- that
     * is deliberate and unrelated, so a near-miss does not dismiss it. */
    shadow.appendChild(wrap);
    document.body.appendChild(hostEl);
  }

  /* "lock" starts with caps lock on, "first" capitalises the first letter
   * then drops to lower case, "off" stays lower case. Set from the admin
   * page; a code typed at a scanner is usually upper case, so lock is the
   * default. */
  function applyCapsMode(el) {
    // Defaults to whatever field is focused, so the three callers that pass
    // nothing -- a live settings change, hide(), and startup -- cannot quietly
    // re-enable shifting on a password box that is still open.
    if (el === undefined) { el = target; }
    var m = (window.__dsOskCaps || 'lock');
    // A password field starts in lower case, whatever the setting says.
    //
    // Neither of the other modes is safe here. "lock" SHOUTS a password, and
    // "first" silently capitalises its first letter -- and a password is the
    // one field where the machine changing what you typed cannot be seen,
    // because the characters are dots. Somebody then types the right password
    // three times and is told three times that it is wrong.
    //
    // Deliberately not a setting. There is no reading of "shift the first
    // letter" that should apply to a secret.
    var t = el && el.getAttribute && (el.getAttribute('type') || '').toLowerCase();
    if (t === 'password') { caps = false; oneShot = false; return; }
    caps = (m === 'lock');
    oneShot = (m === 'first');
  }

  /* The nearest ancestor that actually scrolls.
   *
   * `window.scrollBy` was the whole of this, and on the startup page the window
   * does not scroll: html and body are pinned to 100% and the scrolling happens
   * in a container with overflow-y:auto. So the scroll was a no-op, the
   * keyboard covered the field, and somebody typed a wifi password they could
   * not see -- three times, on a machine whose touchscreen does not work,
   * reported from the field on 2026-08-31. */
  function scrollParent(el) {
    for (var n = el.parentElement; n && n !== document.body; n = n.parentElement) {
      var st;
      try { st = window.getComputedStyle(n); } catch (e) { continue; }
      if (/(auto|scroll)/.test(st.overflowY) && n.scrollHeight > n.clientHeight) {
        return n;
      }
    }
    return null;
  }

  /* Room to scroll INTO.
   *
   * A page that fits its screen has no overflow, so there is nowhere to scroll
   * the field to even once the right element is being scrolled. Padding the
   * bottom by the keyboard's height creates that room, and it is taken away
   * again on hide so nothing is left with a hole in it. */
  var padded = null;
  function makeRoom(el, px) {
    var host = scrollParent(el) || document.body;
    if (padded && padded.node !== host) { clearRoom(); }
    if (!padded) { padded = { node: host, was: host.style.paddingBottom }; }
    host.style.paddingBottom = px + 'px';
    return host;
  }
  function clearRoom() {
    if (!padded) { return; }
    try { padded.node.style.paddingBottom = padded.was || ''; } catch (e) {}
    padded = null;
  }

  function show(el) {
    if (!ENABLED) { return; }
    /* Somebody is typing on real keys. Showing a keyboard over the page they
     * are typing into helps nobody. */
    if (physical) { return; }
    target = el;
    if (!wrap) { build(); }
    applyCapsMode(el);
    relabel();
    wrap.classList.add('on');
    poke();
    setTimeout(function () {
      try {
        var kb = wrap.offsetHeight;
        var host = makeRoom(el, kb + 24);
        var r = el.getBoundingClientRect();
        var lim = window.innerHeight - kb - 12;
        var need = r.bottom - lim + 8;
        if (need > 0) {
          if (host && host !== document.body) { host.scrollTop += need; }
          else { window.scrollBy(0, need); }
        }
        // Whatever the layout, the field must end up visible. If one pass did
        // not do it -- a fixed card, a nested scroller -- ask the browser,
        // which knows about containers this code does not.
        r = el.getBoundingClientRect();
        if (r.bottom > window.innerHeight - kb) {
          el.scrollIntoView({ block: 'center' });
        }
      } catch (err) {}
    }, 30);
  }

  function hide() {
    idleStop();
    if (wrap) { wrap.classList.remove('on'); }
    clearRoom();
    target = null;
    layer = 'abc';
    applyCapsMode();
    if (keysBox) { render(); }
  }

  /* A real keydown puts the keyboard away and keeps it away until a touch.
   *
   * Capture phase, so a page that swallows keydown cannot hide the fact that
   * a keyboard exists. isTrusted is the whole test: this keyboard's own Enter
   * is dispatched from script and is not trusted, so it cannot suppress
   * itself.
   */
  document.addEventListener('keydown', function (e) {
    if (!e.isTrusted) { return; }
    physical = true;
    hide();
  }, true);

  /* ...and any touch says the person is back on the glass. Cleared before
   * focusin runs, which is what lets a tap on a field bring the keyboard
   * back after a scan. */
  ['pointerdown', 'touchstart'].forEach(function (ev) {
    document.addEventListener(ev, function (e) {
      if (!e.isTrusted) { return; }
      physical = false;
      poke();
    }, true);
  });

  /* Typing into the field keeps it up, however the characters got there. */
  document.addEventListener('input', function () { poke(); }, true);

  document.addEventListener('focusin', function (e) {
    if (isField(e.target)) { show(e.target); }
  }, true);

  document.addEventListener('focusout', function (e) {
    setTimeout(function () {
      var a = document.activeElement;
      /* Focus landing inside the keyboard is not the field being finished
       * with. Put it back and stay up. */
      /* Focus inside a shadow root is reported on the HOST element, so
         checking `wrap` alone stopped matching the moment the keyboard moved
         into one -- and a stray focus would then have dismissed it. */
      if (a && ((hostEl && hostEl.contains(a)) || (wrap && wrap.contains(a)))) {
        refocus(); return;
      }
      if (!isField(a)) { hide(); }
    }, 150);
  }, true);

  applyCapsMode();
  if (isField(document.activeElement)) { show(document.activeElement); }
})();
