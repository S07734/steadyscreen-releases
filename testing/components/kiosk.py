...
def doorbell_decide(status, reply, client_id, now, backoff, verify=None, rng=None):
    ...
    if status == 200:
        ...
        if ok and body.get("client_id") == client_id \
                ...
                and isinstance(exp, (int, float)) and not isinstance(exp, bool) \
                and exp > now:
            return "open", 0, body, DB_RETRY_MIN
        return ("wait", HB_MIN_INTERVAL,
                "an open request that did not verify: %s"
                % (why or "not for this display, or expired"), DB_RETRY_MIN)
    if 400 <= status < 500:
...
def heartbeat_payload():
    ...
    private LAN address ("any reason we shouldn't include the local ip"), and
    the public half of its device key, for his one-time approval ("let's
    approve each display once"). Each optional field is left out when it
    cannot be read truthfully, which the server accepts. Nothing else -- not
    the hardware fingerprint, not the page: the server refuses any other field
    with a 422."""
...
<div id=pair style="text-align:center;font:600 1.6rem ui-monospace,monospace;letter-spacing:.08em;opacity:.85"></div>
...
async function tick(){
  ...
  const pc = document.getElementById('pair');
  if(pc) pc.textContent = d.pair_code ? 'Pairing code  ' + d.pair_code : '';
...
class DisplayClient:
        ...
        while True:
            ...
            try:
                ...
                if headers is None:
                    ...
                    if action == "open":
                        # Step 3, the relay, is not built yet: say so, and go
                        # back to listening rather than pretend.
                        self.db.update(state="listening", clock=False,
                                       why="the server asked to open the relay, "
    ...
    def set_option(self, key, value):
        ...
        if key == "refresh_seconds":
            ...
            # A new address sends now rather than at the next interval: whoever
            # changed it is waiting to see it arrive.
            self._hb_wake.set()
...
<div class=card>
 <h2>Remote admin</h2>
 <div style="color:var(--dim);font-size:.85rem">Pairing code</div>
 <div id=paircode style="font:600 2rem ui-monospace,monospace;letter-spacing:.08em;margin:.1rem 0 .5rem">&hellip;</div>
 <dl><dt>Approval</dt><dd id=devappr>&hellip;</dd>
     <dt>Doorbell</dt><dd id=dbstate>&hellip;</dd>
     <dt>Device key</dt><dd id=devkey>&hellip;</dd></dl>
 <div style="color:var(--dim);font-size:.8rem;margin-top:.4rem">
  Approve this display once on the SteadyScreen fleet page: the pairing code
  shown there must match this one. The private key never leaves this display.
 </div>
</div>
...
const TABS = [
  ...
  ['display',  'Display', ['Display', 'Screensaver', 'Keyboard',
                            'Boot splash']],
  ['system',   'System',  ['Remote admin', 'Status', 'Updates', 'Printing', 'Audio',
...
async function refresh(){
  ...
  const dk = document.getElementById('devkey');
  if(dk){
    const k = s.device_key || {};
    const said = {approved: 'approved',
                  pending: 'waiting for approval on the fleet page'}[k.approval]
                 || 'not yet reported to the server';
    dk.textContent = k.fingerprint || 'not created yet';
    const ap = document.getElementById('devappr');
    if(ap) ap.textContent = k.fingerprint ? said : '—';
    const pc = document.getElementById('paircode');
    if(pc) pc.textContent = k.code || '—';
  }
  const dbs = document.getElementById('dbstate');
  if(dbs){
    const d = s.doorbell || {};
    dbs.textContent = d.clock
      ? "refused: this display's clock is wrong -- check the Clock card"
      : (d.state || 'idle') + (d.why ? ' — ' + d.why : '');
  }
