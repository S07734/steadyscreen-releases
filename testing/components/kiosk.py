 ...
 *{box-sizing:border-box}
 ...
 /* Both password fields the same width. They were both class=pwrow but only
    the first had a Show button beside it, so flex made it the narrower of the
    two and they never lined up. */
 /* Two password fields the same width, with one reveal beside the first.
    Both the column AND the row are stated. An item with a definite row but an
    auto column is placed BEFORE the fully-automatic ones, so the button
    claimed row 1 column 1 -- the 1fr column -- and came out half the width of
    the card with both inputs squeezed into the auto column beside it. */
 .pwpair{display:grid;grid-template-columns:1fr auto;gap:.5rem;
   align-items:center;margin:.15rem 0 0}
 .pwpair input{grid-column:1;min-width:0}
 .pwpair button{grid-column:2;grid-row:1;align-self:center}
...
<div class=card>
 ...
 <div class=pwpair>
  ...
  <button type=button class=small onclick="pwToggleBoth(this)"
          aria-pressed=false>Show</button>
...
<div class=card>
 ...
 <div style="border-top:1px solid var(--line);margin-top:1rem;padding-top:.9rem">
  ...
  <div style="color:var(--dim);font-size:.85rem;margin-top:.5rem">
   ...
   machine, not on one a shop depends on.
  </div>
...
async function waitForClient(was){
  const DEADLINE = Date.now() + 180000;   // installs are slow on eMMC
  let seenDown = false;
  while(Date.now() < DEADLINE){
    ...
    } catch(e){ /* still down; that is expected, keep waiting */ }
    if(!s){ seenDown = true; umsg('installing -- the client is restarting...'); continue; }
    // Answering again is not the same as having restarted: the request can
    // land before the browser has gone down. Wait until we have seen it drop,
    // or until the version actually changes.
    if(!seenDown && s.software_version === was) continue;
    const now = s.software_version || 'an unknown version';
    if(was && now === was){
...
function renderUpd(u){
  ...
  e.innerHTML = 'nothing to install'
    ...
    + 'border-radius:50%;width:1.35rem;height:1.35rem;line-height:1;padding:0;'
    + 'cursor:pointer;font:inherit" title="why">i</button>'
...
function pwToggleBoth(btn){
  const a = document.getElementById('adminpw');
  const b = document.getElementById('adminpw2');
  const show = a.type === 'password';
  a.type = b.type = show ? 'text' : 'password';
  btn.textContent = show ? 'Hide' : 'Show';
  btn.setAttribute('aria-pressed', show ? 'true' : 'false');
}
...
def start_admin(cfg):
    ...
    # which is backwards. Flip that one specific pair, once.
    #
    # Only that exact pair. Somebody who deliberately set a different port
    # meant it, and having their machine move underneath them would be worse
    # than the tidiness gained.
    if port == 8080 and int(cfg.get("admin_port_alt") or 0) == 80:
