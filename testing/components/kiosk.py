...
def device_key_code(public):
    """The pairing code the owner compares when approving a display: "XXXX-
 ...
 *{box-sizing:border-box}
 ...
 /* The reinstalled notice: one full-width card, read left-aligned like a
    letter rather than centred like a status. The dashed warn border sets it
    apart from the ordinary setup cards without shouting. */
 .card.reinstalled{flex:1 1 100%;max-width:820px;margin:0 auto;text-align:left;
   border-color:#6a5326;background:#211d15}
 .reinstalled .code{color:var(--fg);text-align:center;margin:.3em 0 0}
 .rtext{color:var(--fg);font-size:clamp(.95rem,1.9vmin,1.25rem);
   margin:.2em 0 .9em;line-height:1.5}
 .rlist{color:var(--fg);font-size:clamp(.9rem,1.8vmin,1.2rem);
   line-height:1.5;margin:0 0 .3em;padding-left:1.3em}
 .rlist li{margin-bottom:.5em}
 .rwarn{color:#e0a33a;font-size:clamp(.85rem,1.6vmin,1.1rem);
   margin:1.1em 0 0;line-height:1.5}
...
<!-- The reinstalled notice. Shown ONLY when the server reports this KNOWN
     display was reinstalled or reset (its new key is pending). There are no
     buttons here on purpose: the display holds no account credentials and
     cannot tell a customer from a passer-by, so the two actions live in the
     customer's own portal. This screen only NAMES them and shows the check
     code to compare. Hidden until the reinstalled state reveals it. -->
<div class=grid id=reinstalledwrap hidden>
  <div class="card reinstalled">
    <div class=label style="color:var(--warn)">This display belongs to an existing account</div>
    <p class=rtext>It looks like this display was reinstalled or reset. To use it
      again, sign in to the account it belongs to &mdash; you&rsquo;ll see a
      notice about this display, with two choices:</p>
    <ul class=rlist>
      <li><b>Reclaim it</b> &mdash; puts it back exactly as it was. Before you do,
        check the code below matches the code your account shows.</li>
      <li><b>Drop from account</b> &mdash; clears it from your account. A setup
        code will then appear on this screen so you can set it up from scratch.</li>
    </ul>
    <div class=label style="margin-top:1.2em">Check code</div>
    <div class=code id=checkcode>&hellip;</div>
    <p class=rwarn>If nobody reinstalled or reset this display, don&rsquo;t choose
      either &mdash; contact us instead.</p>
  </div>
</div>
...
// Swap between the ordinary two-card setup layout and the reinstalled notice.
// Either render path calls the layout it needs, so a display that is Reclaimed
// or Dropped mid-poll falls back to the right screen on its own.
function pairNormalLayout(){
  if(!reinstalledShown){ return; }
  reinstalledShown = false;
  document.getElementById('paircards').hidden = false;
  document.getElementById('reinstalledwrap').hidden = true;
}
...
function pairReinstalled(checkCode){
  ...
  document.getElementById('paircards').hidden = true;
  document.getElementById('reinstalledwrap').hidden = false;
...
async function refresh(){
  ...
  if(ident){
    ...
    if(d.pair_code) bits.push('pairing code ' + esc(d.pair_code));
    if(d.admin_ip){
